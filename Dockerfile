...
ARG ROOT_CONTAINER=nvidia/cuda:11.4.2-runtime-ubuntu20.04
...
COPY GDL_code "${HOME}/GDL_code"
WORKDIR "${HOME}/GDL_code"
...
ENTRYPOINT ["tini", "-g", "--"]
CMD ["start-notebook.sh", "--no-browser", "--NotebookApp.token=''","--NotebookApp.password=''"]